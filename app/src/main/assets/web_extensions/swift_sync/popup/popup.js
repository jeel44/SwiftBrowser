...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const devNameEl = document.getElementById("device-name");
  const fpEl = document.getElementById("device-fingerprint");
  const syncBtn = document.getElementById("btn-sync-now");
  const pairBtn = document.getElementById("btn-pair-device");
  const deviceListEl = document.getElementById("device-list");
  const emptyStateEl = document.getElementById("empty-state");
  const toastEl = document.getElementById("toast");
  const importPhoneBtn = document.getElementById("btn-import-phone");
  const exportPhoneBtn = document.getElementById("btn-export-phone");
  const exportHtmlBtn = document.getElementById("btn-export-html");
  const importHtmlBtn = document.getElementById("btn-import-html");
  const fileHtmlInput = document.getElementById("file-html-input");
  const statBookmarksEl = document.getElementById("stat-bookmarks");
  ...
  const toggleTabs = document.getElementById("toggle-tabs");
  ...
  const copyQrTextBtn = document.getElementById("btn-copy-qr-text");
  const btnQrNextSas = document.getElementById("btn-qr-next-sas");
  const inputInv = document.getElementById("input-invitation");
  const pasteClipBtn = document.getElementById("btn-paste-clip");
  const submitPairBtn = document.getElementById("btn-submit-pair");
  const confirmSasBtn = document.getElementById("btn-confirm-sas");
  ...
  async function loadState() {
    try {
      ...
      fpEl.textContent = myDeviceId.slice(0, 16);
      if (toggleBookmarks) toggleBookmarks.checked = stored.syncBookmarks !== false;
      if (toggleTabs) toggleTabs.checked = stored.syncTabs !== false;
      ...
      statTabsEl.textContent = tabCount.toString();
      statDevicesEl.textContent = devices.length.toString();
      if (stored.lastSyncTime) {
        const d = new Date(stored.lastSyncTime);
        statLastSyncEl.textContent = `Last Synced: ${d.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;
      } else {
        statLastSyncEl.textContent = "Last Synced: Never";
      }
      if (devices.length === 0) {
        ...
        if (deviceListEl) {
          ...
          devices.forEach((dev) => {
            ...
            item.innerHTML = `
              <div>
                <div class="peer-name">${dev.deviceName || "Swift Browser Android Phone"}</div>
  ...
  importPhoneBtn?.addEventListener("click", async () => {
    ...
    const bmkApi = browserApi?.bookmarks;
    if (bmkApi?.search) {
      bmkApi.search({ title: "Swift Browser Phone Bookmarks" }, (results) => {
        const parentFolderId = "1";
        if (!results || results.length === 0) {
          bmkApi.create({
            parentId: parentFolderId,
            title: "Swift Browser Phone Bookmarks"
          }, () => {
            showToast("Imported bookmarks into 'Swift Browser Phone Bookmarks'", "success");
            addActivity("Non-destructive import: Created phone sync folder");
            loadState();
          });
        } else {
          showToast("Phone bookmarks synced in 'Swift Browser Phone Bookmarks'", "success");
  ...
  exportHtmlBtn?.addEventListener("click", () => {
    ...
    if (bmkApi?.getTree) {
      bmkApi.getTree((tree) => {
        let html = '<!DOCTYPE NETSCAPE-Bookmark-file-1>\n<!-- Automatically generated by Swift Sync. -->\n<META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=UTF-8">\n<TITLE>Bookmarks</TITLE>\n<H1>Bookmarks</H1>\n<DL><p>\n';
        ...
        a.href = url;
        a.download = "swift-bookmarks-backup-" + (/* @__PURE__ */ new Date()).toISOString().slice(0, 10) + ".html";
        a.click();
    ...
    reader.onload = (evt) => {
      ...
      if (content && bmkApi?.create) {
        bmkApi.create({
          ...
          showToast("Safely imported " + imported + " bookmarks into isolated folder", "success");
          addActivity("Imported " + imported + " bookmarks from file");
          loadState();
        });
      }
    };
  ...
  async function generateAndRenderQr() {
    try {
      ...
      currentInvitationJson = JSON.stringify(invitation);
      if (outputQrCode) outputQrCode.value = currentInvitationJson;
      if (qrCanvas) {
        await import_qrcode.default.toCanvas(qrCanvas, currentInvitationJson, {
          width: 140,
          margin: 1,
          color: {
            dark: "#000000",
  ...
  submitPairBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      if (!remoteDeviceId || !remotePubKey) {
        showToast("Invalid token: Missing device ID or public key", "error");
      ...
      const newDevice = {
        ...
        deviceName: inv.deviceName || "Swift Browser Android Phone",
      ...
      for (const host of candidateHosts) {
        try {
          ...
          if (resp.ok) {
            console.log(`[Swift Sync] Phone successfully acknowledged mutual pairing at ${host}:${port}`);
      ...
      showToast("Failed to parse token. Ensure valid JSON.", "error");
    } finally {
      submitPairBtn.textContent = "Verify & Link";
    }
  });
