...
class SwiftBrowserApplication : Application(), coil.ImageLoaderFactory {
    ...
    override fun newImageLoader(): coil.ImageLoader {
        ...
        val okHttpClient = baseClient.newBuilder()
            .addInterceptor { chain ->
                ...
                val referer = com.swiftbrowser.fast.secure.browser.ImageGrabberUtils.resolveReferer(originalUrl)
                ...
                if (response == null || !response.isSuccessful) {
                    ...
                    val alternateUrls = com.swiftbrowser.fast.secure.browser.ImageGrabberUtils.getCandidateAlternateUrls(originalUrl)
                    android.util.Log.d("ImageFallback", "Candidates (${alternateUrls.size}): $alternateUrls")
                    for (altUrl in alternateUrls) {
                        try {
                            val altReferer = com.swiftbrowser.fast.secure.browser.ImageGrabberUtils.resolveReferer(altUrl)
    ...
    override fun onCreate() {
        ...
        appContext = this
        com.swiftbrowser.fast.secure.sync.notification.SyncNotificationManager.createNotificationChannel(this)
...
object SyncCoordinatorHolder {
    @Volatile
    var coordinator: com.swiftbrowser.fast.secure.sync.coordinator.SyncCoordinator? = null
        private set

    fun getOrCreate(baseDir: java.io.File, collection: com.swiftbrowser.fast.secure.bookmarks.model.BookmarkCollection): com.swiftbrowser.fast.secure.sync.coordinator.SyncCoordinator {
        return coordinator ?: synchronized(this) {
            coordinator ?: com.swiftbrowser.fast.secure.sync.coordinator.SyncCoordinator(
                baseDir = baseDir,
