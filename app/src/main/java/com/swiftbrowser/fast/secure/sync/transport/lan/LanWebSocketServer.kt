package com.swiftbrowser.fast.secure.sync.transport.lan

import android.util.Log
import com.swiftbrowser.fast.secure.bookmarks.model.BookmarkCollection
import com.swiftbrowser.fast.secure.bookmarks.storage.saveBookmarks
import com.swiftbrowser.fast.secure.sync.conflict.ConflictEngine
import com.swiftbrowser.fast.secure.sync.core.SyncBridge
import com.swiftbrowser.fast.secure.sync.crypto.DeviceKeyManager
import com.swiftbrowser.fast.secure.sync.crypto.EncryptedEnvelope
import com.swiftbrowser.fast.secure.sync.crypto.TrustManager
...
class LanWebSocketServer(
        ...
        return try {
            ...
            if (operationsArray != null) {
                for (i in 0 until operationsArray.length()) {
                    ...
                    val hlc = com.swiftbrowser.fast.secure.sync.model.Hlc.parse(hlcStr)
                    val opType = com.swiftbrowser.fast.secure.sync.model.SyncOpType.valueOf(opTypeStr)
                    val entityType = com.swiftbrowser.fast.secure.sync.model.SyncEntityType.valueOf(entityTypeStr)

                    val bookmarkPayload = if (opObj.has("bookmarkPayload")) {
                        val bp = opObj.getJSONObject("bookmarkPayload")
                        com.swiftbrowser.fast.secure.sync.model.BookmarkPayload(
                    ...
                    val folderPayload = if (opObj.has("folderPayload")) {
                        ...
                        com.swiftbrowser.fast.secure.sync.model.FolderPayload(
    ...
    fun processPairingPayload(jsonString: String): String {
        return try {
            ...
            val nonce = if (nonceStr.isNotEmpty()) Base64.getDecoder().decode(nonceStr) else com.swiftbrowser.fast.secure.sync.crypto.CryptoEngine.generateRandomNonce(16)

            val sas = com.swiftbrowser.fast.secure.sync.crypto.CryptoEngine.deriveSasCode(myPubKey, peerPubKey, nonce)

            val trusted = com.swiftbrowser.fast.secure.sync.crypto.TrustedDevice(
